...
def linear(x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor | None = None,
           scale_fmt: str | None = None) -> torch.Tensor:
    """
...
def apply_rotary_emb(x: torch.Tensor, freqs_cis: torch.Tensor, interleaved: bool = True) -> torch.Tensor:
    ...
    """
    # x.shape == (bsz, seq_len, n_heads or 1, rope_head_dim)
    dtype = x.dtype
    shape = x.shape
    if not interleaved:
        # x.shape == (baz, seq_len, n_heads or 1, rope_head_dim // 2, 2)
        x = x.view(*shape[:-1], 2, -1).transpose(-1, -2).contiguous()
    # x.shape == (bsz, seq_len, n_heads or 1, rope_head_dim // 2)
    x = torch.view_as_complex(x.float().view(*shape[:-1], -1, 2))

    # freqs_cis.shape == (1, seq_len, 1, rope_head_dim // 2)
    freqs_cis = freqs_cis.view(1, x.size(1), 1, x.size(-1))
    # y.shape == (bsz, seq_len, n_heads or 1, rope_head_dim // 2, 2).flatten(3) == (bsz, seq_len, n_heads or 1, rope_head_dim)
...
class Indexer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, qr: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: torch.Tensor | None):
        bsz, seqlen, _ = x.size()
        end_pos = start_pos + seqlen
        # indexer中的q基于mla的q_lora_rank部分进行计算。
        q = self.wq_b(qr)
        q = q.view(bsz, seqlen, self.n_heads, self.head_dim)
        q_pe, q_nope = torch.split(q, [self.rope_head_dim, self.head_dim - self.rope_head_dim], dim=-1)
        # rope in indexer is not interleaved
        q_pe = apply_rotary_emb(q_pe, freqs_cis, False)
        # q.shape == (bsz, seq_len, n_heads, head_dim)
        q = torch.cat([q_pe, q_nope], dim=-1)

        k = self.wk(x)
        k = self.k_norm(k)
        # k_pe.shape == (bsz, seq_len, rope_head_dim), k_nope.shape == (bsz, seq_len, head_dim - rope_head_dim)
        k_pe, k_nope = torch.split(k, [self.rope_head_dim, self.head_dim - self.rope_head_dim], dim=-1)
        # rope in indexer is not interleaved
        # k_pe.shape == (bsz, seq_len, rope_head_dim), freqs_cis.shape == (seq_len, rope_head_dim // 2)
        k_pe = apply_rotary_emb(k_pe.unsqueeze(2), freqs_cis, False).squeeze(2)
        # k.shape == (bsz, seq_len, head_dim)
        ...
        # weights.shape == (bsz, seq_len, n_heads)
        weights = self.weights_proj(x.float()) * self.n_heads ** -0.5
        # weights.shape == (bsz, seq_len, n_heads, 1)
        weights = weights.unsqueeze(-1) * q_scale * self.softmax_scale
        # index_score.shape == (bsz, seq_len of q in prefill or 1 in decode, end_pos)
        index_score = fp8_index(q_fp8.contiguous(), weights, self.k_cache[:bsz, :end_pos].contiguous(), self.k_scale_cache[:bsz, :end_pos].contiguous())
        if mask is not None:
            index_score += mask
        # topk_indices.shape == (bsz, seq_len or 1, index_topk)
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, residual: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: torch.Tensor | None) -> torch.Tensor:
        """
