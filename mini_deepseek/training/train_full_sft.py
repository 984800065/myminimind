"""
MiniDeepSeek Full SFT 入口：get_sft_config() 加载参数，DDP + 混合精度 + swanlab。
"""
...
from mini_deepseek.config import SFTConfig, get_sft_config
...
def train_epoch(
    ...
    SFTDataset 已将 system、user 和 padding 位置的 label 设为 `-100`，因此
    模型只对 assistant 回复计算语言模型损失。其余前向、梯度累积、参数更新、
    指标记录和 checkpoint 逻辑与预训练保持一致。

    续训时，`loader` 已跳过当前 epoch 中完成的 batch，`start_step` 保留其
    原始编号，使进度条、日志和 checkpoint 中的 step 在恢复前后保持连续。

    Args:
        cfg: SFT 配置，包含设备、梯度累积、日志和 checkpoint 等参数。
    ...
    for step, (input_ids, labels) in enumerate(pbar, start=start_step):
        ...
        # main() 在 init_distributed() 后按 LOCAL_RANK 设置 cfg.device，此处将当前 rank 的 batch 移到对应 GPU。
        input_ids = input_ids.to(cfg.device)
        labels = labels.to(cfg.device)

        # 原生 CUDA 训练使用 autocast；CPU 和自行管理混合精度的 DeepSpeed 使用空上下文。
        with autocast_ctx:
            # TODO: SFTDataset 暂未返回 attention_mask，MoE 辅助路由损失仍会统计 padding token；后续需让模型侧按 mask 排除 padding。
...
def train(
    ...
    Args:
        cfg: SFT 配置，包含 epoch、batch size、数据加载和设备等参数。
        model: 待训练模型，可以是原始模型、DDP 包装模型或 DeepSpeed 引擎。
        optimizer: 负责更新模型参数的优化器。
        lr_scheduler: 按 optimizer update 调整学习率的调度器。
        scaler: 原生混合精度训练使用的梯度缩放器；DeepSpeed 模式下为 `None`。
        ...
        train_dataset: 已将非 assistant token 屏蔽的 SFT 数据集。
...
def main():
    ...
    本入口的训练、分布式、DeepSpeed、续训和 checkpoint 控制流与预训练一致。
    两者的实际行为差异主要来自 `SFTConfig` 的默认参数和 `SFTDataset` 生成的
    labels；后者只保留 assistant 回复作为监督目标，并屏蔽 system、user 和
    padding token。需要理解 SFT 与预训练的区别时，应优先查看这两个模块。
    """
    # 加载运行配置。配置由代码默认值、环境变量、可选配置文件和命令行参数分层合并。
    cfg: SFTConfig = get_sft_config()
    if cfg.use_deepspeed and cfg.use_compile:
        raise ValueError("当前 SFT 入口暂不建议同时启用 DeepSpeed 与 torch.compile，请先关闭其中一个。")
    if cfg.deepspeed_tensor_parallel_size != 1:
        raise ValueError("当前 SFT 入口尚未适配 DeepSpeed Tensor Parallel，deepspeed_tensor_parallel_size 必须为 1。")
    ...
    # 准备输出目录、模型配置和续训元数据。创建模型前先解析 tokenizer，并把词表大小及特殊 token ID 同步到 MiniDeepSeekConfig。
    ...
    resume_available = ckp_data is not None or ds_resume_meta is not None

    # 加载基础权重或恢复 checkpoint 前，读取权重旁同名的 .config.json，确认当前模型配置与权重一致；不一致时直接报错，避免加载错误结构的权重继续训练。
    validate_sft_model_config(cfg, lm_config, resume_available=resume_available)
    ...
    # 构建模型、SFT 数据集和优化器；SFTDataset 只保留 assistant 回复位置的监督标签。
    model, tokenizer = init_model(
        cfg=cfg,
        lm_config=lm_config,
        tokenizer=tokenizer,
        from_weight="none" if resume_available else None,
    )
    if cfg.use_compile:
    ...
    if cfg.use_deepspeed:
        ...
        if ds_resume_meta is not None and client_state is None:
            raise RuntimeError("检测到 DeepSpeed resume 元数据，但未能加载对应 checkpoint，请检查 checkpoint 目录是否完整。")
    ...
    # 训练主循环。SFTDataset 已屏蔽非 assistant token，其余流程与预训练保持一致。
    train(
        cfg,
        model,
        optimizer,
